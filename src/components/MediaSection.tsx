          ...
          {mediaList.map((item) => (
            ...
            >
              ...
              {tipsHistories[item.id!] && tipsHistories[item.id!].length > 0 && (
                <div className="mt-6 pt-4 border-t border-dashed border-stone-100 space-y-2 max-h-32 overflow-y-auto">
                  <p className="text-[10px] font-black uppercase text-stone-400 tracking-wider">最近の応援メッセージ:</p>
                  <div className="space-y-1.5">
                    {tipsHistories[item.id!].map((tip) => (
                      <div key={tip.id} className="bg-stone-50 p-2.5 rounded-xl border border-stone-200 flex items-start gap-2.5">
                        <div className="bg-artistic-pink/10 w-6 h-6 rounded-lg text-artistic-pink flex items-center justify-center shrink-0 border border-artistic-pink/20 text-[10px] font-bold">
                          🪙
                        </div>
                        <div className="text-[11px] leading-relaxed">
                          <p className="font-black text-stone-600 block">
                            {tip.backerName} <span className="text-artistic-pink">¥{tip.amount} 投下!</span>
                          </p>
                          {tip.cheerMessage && (
                            <p className="text-stone-500 font-bold italic mt-0.5">「{tip.cheerMessage}」</p>
                          )}
                        </div>
                      </div>
                    ))}
